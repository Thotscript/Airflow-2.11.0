...
WINDOW_SEC = 60
MAX_CALLS = 90  # Limite de segurança abaixo de 100/min
_call_times = deque()
...
# ---- POST com retry/backoff customizado ----
def post_with_retry(session, payload, retries=6, base_delay=1, timeout=60):
    delay = base_delay
    headers = {"Content-Type": "application/json"}
    
    for attempt in range(retries):
        throttle() # Garante que não bata o rate limit antes de enviar
        ...
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                # Verifica se a API retornou null por excesso de chamadas silencioso
            ...
            # Erros de servidor ou rate limit oficial (429)
            ...
            return resp # Outros erros (400, 401) não repetimos
...
def fetch_calendar(session: requests.Session, unit_id: int, startdate: str, enddate: str) -> list:
    ...
    try:
        # Substituído session.post por post_with_retry
        resp = post_with_retry(session, payload)
        resp.raise_for_status()
        data = resp.json()
        
        # DEBUG DETALHADO (Mantido original)
...
def main():
    ...
    for unit_id in unit_ids:
        ...
        for month_info in months_2026:
            ...
            occupancy_rate, days_occupied, days_in_month = calculate_occupancy(blocked, month_info['startdate'])
            
            results.append({
                'unit_id': unit_id, 'year': month_info['year'], 'month': month_info['month'],
                'month_str': month_info['month_str'], 'occupancy_rate': round(occupancy_rate, 4),
                'days_occupied': days_occupied, 'days_in_month': days_in_month, 'extraction_date': extraction_date
            })
...
with DAG(dag_id="OVH-tb_occupancy_houses", start_date=pendulum.datetime(2025, 12, 19, 8, 0, tz=SP_TZ), schedule="0 2 * * *", catchup=False, tags=["Tabelas - OVH", "Ocupacao"]) as dag:
